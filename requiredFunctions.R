							...
							if(exonId == exonNum){
								...
								if(!is.na(pamSites[[i]][[j]][[k]][[l]]) && !is.null(pamSites[[i]][[j]][[k]][[l]])){
									...
									if(contextCondition){
										...
										scoreFrame <- calculateMENTHU2(as.character(context), cutSite = 40, weight = 20)
										
										#If the score is greater than the threshold, report it
										if(scoreFrame$MENTHUscore >= threshold){
											
											#Get the CRISPR target sequence required to target this site
											if(strandId == "forward"){
												crispr <- substr(scoreFrame$seq, 
																				 (40 + -cutDistList[i] - 19), 
																				 (40 + -cutDistList[i] + nchar(toolTypeI)))
											} else if(strandId == "complement"){
												crispr <- as.character(substr(Biostrings::reverseComplement(DNAString(scoreFrame$seq)), 
																											(40 + -cutDistList[i] - 19), 
																											(40 + -cutDistList[i] + nchar(toolTypeI))))
											}
											...
											formFrame  <- data.frame(Target_Sequence = crispr, 
																							 MENTHU_Score    = round(abs(scoreFrame$MENTHUscore), digits = 2), 
																							 Frame_Shift     = scoreFrame$frameShift,
																							 Tool_Type       = toolTypeI, 
																							 Strand          = strandId, 
																							 Exon_ID         = exonNum, 
										...
										if(!is.na(pamSites[[i]][[j]][[k]][[l]]) && !is.null(pamSites[[i]][[j]][[k]][[l]])){
											...
											if(contextCondition){
												...
												if(scoreFrame$MENTHUscore >= threshold){
													...
													#Get the CRISPR target sequence required to target this site
													if(strandId == "forward"){
														crispr <- substr(scoreFrame$seq, (40 + -cutDistList[i] - 19), (40 + -cutDistList[i] + nchar(toolTypeI)))
													} else if(strandId == "complement"){
														crispr <- as.character(substr(Biostrings::reverseComplement(DNAString(scoreFrame$seq)), (40 + -cutDistList[i] - 19),  (40 + -cutDistList[i] + nchar(toolTypeI))))
													}
